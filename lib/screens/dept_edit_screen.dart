...
class DeptEditScreen extends StatefulWidget {
  final FieldItem fieldItem;
  final Dept dept;
  final List<TypeOfWork> typeOfWorkList;

  const DeptEditScreen({
    super.key,
  ...
  @override
  State<DeptEditScreen> createState() => _DeptEditScreenState();
}

class _DeptEditScreenState extends State<DeptEditScreen> {
  final AuthService _authService = AuthService();
  late final TextEditingController _nameController;

  late int _selectedTypeOfWorkID;
  bool _isSaving = false;

  @override
  void initState() {
    super.initState();
    _nameController = TextEditingController(text: widget.dept.deptName);
    _selectedTypeOfWorkID = widget.dept.typeOfWorkID;
  }
  ...
  Future<void> _onTypeOfWorkTap() async {
    final picked = await showModalBottomSheet<int>(
      ...
      builder: (_) => TypeOfWorkPickerSheet(
        ...
        currentTypeOfWorkID: _selectedTypeOfWorkID,
      ),
    );
  ...
  Future<void> _save() async {
    ...
    try {
      final response = await _authService.updateDept(
        companyID: widget.fieldItem.companyID,
        fieldID: widget.fieldItem.fieldID,
        deptID: widget.dept.deptID,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SafeArea(
        child: Column(
          children: [
            _DeptEditHeader(onBack: () => Navigator.of(context).pop()),
            Expanded(
...
class _DeptEditHeader extends StatelessWidget {
  final VoidCallback? onBack;

  const _DeptEditHeader({this.onBack});
        ...
        children: [
          GestureDetector(
            ...
            child: const SizedBox(
              width: 40,
              height: 40,
              child: Icon(Icons.chevron_left, color: Color(0xFF101828), size: 28),
            ),
          ),
